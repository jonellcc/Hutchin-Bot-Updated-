    ...
    if (adminConfig.adminUIDs.includes(event.senderID)) {
      ...
      if (action === "add") {
        if (role === "admin" && !adminConfig.adminUIDs.includes(targetUID)) {
          ...
          let userName = await global.getUser(targetUID);

          if (userName) {
            return api.sendMessage(`🛡️ 𝗔𝗱𝗺𝗶𝗻 𝗔𝗱𝗱𝗲𝗱\n${global.line}\nSuccessfully added new admin ${userName.name}`, event.threadID, () => {
              process.exit(1);
            });
          }
          ...
          let userName = await global.getUser(targetUID);
          if (userName) {
            return api.sendMessage(`👮 𝗠𝗼𝗱𝗲𝗿𝗮𝘁𝗼𝗿 𝗔𝗱𝗱𝗲𝗱\n${global.line}\nSuccessfully added new moderator ${userName.name}`, event.threadID, () => {
              process.exit(1);
            });
          }
        } else {
          let userName = await global.getUser(targetUID);
          if (userName) {
            return api.sendMessage(`${userName.nams} is already an ${role}.`, event.threadID);
          }
        }
      }
      ...
      if (action === "remove" && role === "admin") {
        if (adminConfig.adminUIDs.includes(targetUID)) {
          ...
          let userName = await global.getUser(targetUID);
          if (userName) {
            return api.sendMessage(`🛡️ 𝗔𝗱𝗺𝗶𝗻 𝗥𝗲𝗺𝗼𝘃𝗲𝗱\n${global.line}\nSuccessfully removed admin ${userName.name}`, event.threadID, () => {
              process.exit(1);
            });
          }
        }
      ...
      for (let adminUID of adminConfig.adminUIDs) {
        let adminName = await global.getUser(adminUID);
        if (adminName) adminList.push(`[ ${adminUID} ] ${adminName.name}`);
      }

      let moderatorList = [];
      if (adminConfig.moderatorUIDs) {
        for (let modUID of adminConfig.moderatorUIDs) {
          let modName = await global.getUser(modUID);
          if (modName) moderatorList.push(`[ ${modUID} ] ${modName.name}`);
        }
      }
