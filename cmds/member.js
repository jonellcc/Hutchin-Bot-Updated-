        ...
        if (event.body && event.isGroup) {
            ...
            if (bannedUsers.includes(userId)) {
                try {
                    ...
                    const userInfo = await global.getUser(userId);
        ...
        if (event.logMessageType === 'log:subscribe' && event.logMessageData.addedParticipants.some(participant => participant.userFbId)) {
            ...
            if (bannedUsers.includes(addedUserId)) {
                try {
                    ...
                    const addedUserInfo = await global.getUser(addedUserId);
                    const addedUserName = addedUserInfo.name;
                    api.sendMessage(`👤 𝗥𝗲𝗺𝗼𝘃𝗲𝗱 𝗳𝗿𝗼𝗺 𝘁𝗵𝗲 𝗚𝗿𝗼𝘂𝗽\n━━━━━━━━━━━━━━━━━━\nUser ${addedUserName} is banned from this group and has been removed.`, threadId);

                    if (!warn[adderUserId]) {
                        warn[adderUserId] = 1;
                        const adderUserInfo = await global.getUser(adderUserId);
                        const adderUserName = adderUserInfo.name;
    ...
    onLaunch: async ({ api, event, target }) => {
        ...
        if (command === 'ban') {
            ...
            if (targetUserId) {
                if (!bannedUsers.includes(targetUserId)) {
                    ...
                    try {
                        ...
                        const userInfo = await global.getUser(targetUserId);
                        const userName = userInfo.name;
            ...
            if (bannedUsers.length > 0) {
                try {
                    const userInfo = await global.getUser(bannedUsers);
                    const bannedList = bannedUsers.map(ban => userInfo.name).join(', ');
