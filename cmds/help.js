    ...
    onLaunch: async function ({ api, event, target }) {
        ...
        const visibleCommandFiles = commandFiles.filter(file => {
            try {
                ...
                console.warn(`[HELP CMD] Skipping "${file}" due to error:\n${err.message}`);
                return false;
            }
        });
        ...
        if (!isNaN(page)) {
            ...
            displayedCommands.forEach(file => {
                try {
                    ...
                    // Skip this command if error happens again
                }
            });
            ...
            if (commandFile) {
                try {
                    ...
                    const helpMessage =
                        ...
                        `│✧ Usages: ${commandInfo.usages !== undefined ? commandInfo.usages : "Unknown"}\n` +
                        `╰───────────◊`;

                    return api.shareContact(helpMessage, api.getCurrentUserID(), event.threadID);
                } catch (err) {
                    return api.sendMessage(`Failed to load command "${commandName}". Possible syntax error.`, event.threadID);
                }
            } else {
