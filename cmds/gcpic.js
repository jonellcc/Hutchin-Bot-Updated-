...
module.exports = {
    name: "gcpic",
    info: "Generate a YouTube thumbnail-style canvas with ALL group members' profile pictures, group name, total members, and admin count 🖼️",
    dev: "Hridoy",
    onPrefix: true,
    dmUser: false,
    nickName: ["groupcanvas", "gcanvas"],
    usages: "Use {pn}gcpic to generate a stylish group canvas image",
    cooldowns: 5,
    ...
    onLaunch: async function ({ api, event, actions }) {
        ...
        try {
            if (!event || !threadID || !messageID) {
                return actions.reply("❌ Invalid event data.");
            }
            ...
            for (let row = 0; row < rows; row++) {
                for (let col = 0; col < cols; col++) {
                    ...
                    const borderColors = ['#ff5722', '#2196f3', '#4caf50', '#e91e63', '#ff9800', '#9c27b0'];
            ...
            const msg = {
                body: `✨ Here's your group canvas showing all ${totalMembers} members of "${groupName}"! ✨`,
            ...
            await actions.react("❌");
            await actions.reply(`⚠️ Error: ${err.message}`);
