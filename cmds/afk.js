...
const afkDBPath = path.join(__dirname, 'cache', 'afk.json');
let afkDB = {};
...
if (fs.existsSync(afkDBPath)) {
  afkDB = JSON.parse(fs.readFileSync(afkDBPath));
}

function saveDB() {
  fs.writeFileSync(afkDBPath, JSON.stringify(afkDB));
}
...
module.exports = {
  ...
  onLaunch: async function ({ api, event, actions, target }) {
    ...
    const userInfo = await api.getUserInfo(userID, false);
  ...
  noPrefix: async function ({ api, event, actions }) {
    ...
    try {
      ...
      if (afkDB[userID]) {
        ...
        const timeText = duration.asHours() >= 1
