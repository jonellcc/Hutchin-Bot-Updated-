...
module.exports = {
  ...
  onLaunch: async function ({ api, event, target }) {
    ...
    exec(command, (error, stdout, stderr) => {
      if (error) {
        return api.editMessage(`❌ Error:\n${error.message}`, msg.messageID);
      }
      if (stderr) {
        return api.editMessage(`⚠️ Stderr:\n${stderr}`, msg.messageID);
      }
      api.editMessage(stdout || "✅ Done with no output.", msg.messageID);
    });
  }
};