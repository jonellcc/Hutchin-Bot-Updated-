...
onLaunch: async function ({ api, event, target }) {
 ...
 const lod = await api.sendMessage("Please Wait.....", event.threadID, event.messageID);
        ...
        const apiUrl = `https://ccprojectsapis.zetsu.xyz/api/pin?title=${keySearch}&count=${numberSearch}`;
        ...
        for (let i = 0; i < Math.min(numberSearch, data.length); i++) {
            ...
            const imgPath = path.join(__dirname, "pin", `${i + 1}.jpg`);
    ...
    api.unsendMessage(lod.messageID);
        ...
        await fs.remove(path.join(__dirname, "pin"));
