  ...
  sharedWebSocket.on("message", async (data) => {
    try {
      ...
      const stockData = {
        ...
        seed: stock.seed,
        egg: stock.egg,
        travelingmerchant: stock.travelingmerchant
      };

      Object.keys(stockData).forEach(cat => {
        if (stockData[cat]?.items) {
      ...
      const processAndSend = async (id, isGroup = false) => {
        ...
        let sections = [];
        let matchCount = 0;
        const triggerItems = ["master sprinkler", "sugar apple", "burning bud", "ember lily", "beanstalk", "godly sprinkler", "giant pinecone"];
        ...
        if (stockData.travelingmerchant && Array.isArray(stockData.travelingmerchant.items) && stockData.travelingmerchant.items.length > 0) {
          checkAndAdd("🚚 𝗧𝗿𝗮𝘃𝗲𝗹𝗶𝗻𝗴 𝗠𝗲𝗿𝗰𝗵𝗮𝗻𝘁", stockData.travelingmerchant, false);

          // Mention all members if merchant has stock
          if (isGroup) {
            try {
              ...
              userIDs.forEach(uid => {
                const tag = "\u200B";
                mentions.push({ id: uid, tag, fromIndex: body.length });
                body += tag;
              });

              body += "\n📦 𝗧𝗿𝗮𝘃𝗲𝗹𝗶𝗻𝗴 𝗠𝗲𝗿𝗰𝗵𝗮𝗻𝘁 𝗵𝗮𝘀 𝗮𝗿𝗿𝗶𝘃𝗲𝗱! 🚚 Check the stock now!";
              await api.sendMessage({ body, mentions }, id);
            } catch (err) {
              console.error("Merchant mention error:", err);
            }
          }
        } else {
          sections.push("🚚 𝗧𝗿𝗮𝘃𝗲𝗹𝗶𝗻𝗴 𝗠𝗲𝗿𝗰𝗵𝗮𝗻𝘁:\n🚫 MERCHANT is not spawning.");
        }
