...
function parseAndCheckLogin(ctx, http, retryCount = 0) {
  ...
  return function (data) {
    function any() {
      if (data.statusCode >= 500 && data.statusCode < 600) {
        ...
        if (data.request.headers["content-type"].split(";")[0] === "multipart/form-data") {
          return delay(retryTime)
            .then(() => http.postFormData(url, ctx.jar, data.request.formData, data.request.qs, ctx.globalOptions, ctx))
            .then(parseAndCheckLogin(ctx, http, retryCount));
        } else {
          return delay(retryTime)
            .then(() => http.post(url, ctx.jar, data.request.form, ctx.globalOptions, ctx))
            .then(parseAndCheckLogin(ctx, http, retryCount));
        }
      }
      ...
      let res = null;
      // --- START OF FIX ---
      // Check if the body is already a parsed object from axios
