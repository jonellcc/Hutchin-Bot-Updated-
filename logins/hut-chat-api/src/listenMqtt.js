...
function listenMqtt(defaultFuncs, api, ctx, globalCallback) {
	...
	const chatOn = true;// ctx.globalOptions.online;
	...
	if (ctx.mqttEndpoint) {
		...
		host = `wss://edge-chat.messenger.com/chat?region=${ctx.region.toLowerCase()}&sid=${sessionID}&cid=${GUID}`;
	} else {
		host = `wss://edge-chat.messenger.com/chat?sid=${sessionID}&cid=${GUID}`;
	}
	...
	const options = {
		...
		wsOptions: {
			headers: {
				...
				'User-Agent': ctx.globalOptions.userAgent || 'facebookexternalhit/1.1 (+http://www.facebook.com/externalhit_uatext.php)',
