...
let adminConfig = { adminUIDs: [], notilogs: true, logs: true };
let usersData = {};
let threadsData = {};
...
const notifyAdmins = async (api, threadID, action, senderID) => {
    if (adminConfig.notilogs) {
        ...
        const addedOrKickedBy = await global.getUser(senderID);
...
const logChatRecord = async (api, event) => {
    if (!adminConfig.logs) return;

    const threadID = event.threadID;
    const senderID = event.senderID;
    const userName = await global.getUser(senderID);
    const groupName = await getGroupName(api, threadID);
    ...
    if (event.body) {
        ...
        console.log(`${boldText(gradientText(`┣➤ Group ID: ${threadID}`))}`);
        console.log(`${boldText(gradientText(`┣➤ User ID: ${senderID}`))}`);
        console.log(`${boldText(gradientText(`┣➤ Content: ${userName.name} sent an attachment or stickers`))}`);
        console.log(`${boldText(gradientText(`┣➤ Time: ${time}`))}`);
        console.log(gradientText("┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛"));
    }
};
